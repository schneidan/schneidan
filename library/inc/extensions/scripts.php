...
function reactor_register_scripts() {
	...
	//Added DJS
	wp_register_script('functions-js', get_stylesheet_directory_uri() . '/library/js/functions.js', array( 'jquery' ), false, true);
}
...
function reactor_enqueue_scripts() {
	if ( !is_admin() ) { 
		...
		if ( is_page_template('page-templates/sponsor-content.php') ) {
			wp_enqueue_script('sponsored-funcs');
		}
